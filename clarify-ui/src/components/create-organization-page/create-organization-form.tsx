...
function CreateOrganizationForm(): JSX.Element {
  ...
  return (
    <Form
      ...
      autoComplete='off'
      onFinish={onFinish}
      className='clarify-create-organization-form'
      layout='vertical'
    >
      <Form.Item
        hasFeedback
      ...
      <Form.Item>
        <Space className='clarify-create-organization-form-buttons-block' align='end'>
          <Button className='clarify-cancel-new-organization-button' onClick={() => history.goBack()}>Cancel</Button>
          <Button className='clarify-submit-new-organization-button' loading={creating} disabled={creating} htmlType='submit' type='primary'>
            Submit
          </Button>
        </Space>
      </Form.Item>
