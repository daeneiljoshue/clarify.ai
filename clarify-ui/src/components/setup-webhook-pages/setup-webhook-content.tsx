...
import { getCore, Webhook } from 'clarify-core-wrapper';
...
function SetupWebhookContent(props: Props): JSX.Element {
    ...
    const handleSubmit = useCallback(async (): Promise<Webhook | null> => {
        try {
            ...
            let notificationConfig = {
                ...
                className: 'clarify-notification-update-webhook-success',
            };
            if (webhook) {
                ...
                notificationConfig = {
                    ...
                    className: 'clarify-notification-create-webhook-success',
                };
    ...
    return (
        <Row justify='start' align='middle' className='clarify-setup-webhook-content'>
            <Col span={24}>
            ...
            <Col span={24}>
                ...
                >
                    ...
                    >
                        <Checkbox>
                            <Text className='clarify-text-color'>Enable SSL</Text>
                        </Checkbox>
                    </Form.Item>
                    <Form.Item
                        help='CLARIFY will deliver events for active webhooks only'
                        name='isActive'
                        valuePropName='checked'
                    >
                        <Checkbox>
                            <Text className='clarify-text-color'>Active</Text>
                        </Checkbox>
                    </Form.Item>
                    <Form.Item
                        name='eventsMethod'
                    ...
                    {
                        showDetailedEvents && (
                            <Row className='clarify-webhook-detailed-events'>
                                {groupEvents(webhookEvents).map((event: string, idx: number) => (
                                    <Col span={8} key={idx}>
                                        ...
                                        >
                                            <Checkbox>
                                                <Text className='clarify-text-color'>{event}</Text>
                                            </Checkbox>
                                        </Form.Item>
                                    </Col>
                                ))}
            ...
            <Col span={24}>
                <Row justify='end'>
                    <Col>
                        <Button className='clarify-submit-webhook-button' type='primary' onClick={handleSubmit}>
                            Submit
                        </Button>
                    </Col>
                </Row>
