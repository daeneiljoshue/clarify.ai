...
import { AnnotationConflict, ObjectState, QualityConflict } from 'clarify-core-wrapper';
...
function ReviewContextMenu({
    ...
    return (
        <Menu onClick={onClick} selectable={false} className='clarify-canvas-context-menu' style={{ top, left }}>
            <Menu.Item className='clarify-context-menu-item' key={ReviewContextMenuKeys.OPEN_ISSUE}>
                Open an issue ...
            </Menu.Item>
            {conflict ? (
                <Menu.Item
                    className='clarify-context-menu-item clarify-quick-issue-from-conflict'
...
export default function CanvasContextMenu(props: Props): JSX.Element | null {
    ...
    if (Number.isInteger(contextMenuParentID)) {
        return ReactDOM.createPortal(
            <div className='clarify-canvas-context-menu' style={{ top, left }}>
