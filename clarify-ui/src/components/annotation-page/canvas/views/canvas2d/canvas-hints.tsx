...
import { CanvasHint } from 'clarify-canvas-wrapper';
    ...
    public render(): JSX.Element | null {
        ...
        if (hints && !hiddenHints[topic]) {
            const blocks = hints.map(({
                ...
                if (type === 'text') {
                    return (
                        <div key={idx} className={`clarify-canvas-hints-block ${className || ''}`}>
                            { Icon }
            ...
            return (
                <div style={{ filter: 'none' }} className='clarify-canvas-hints-container'>
                    { blocks }
                    { topic !== FORCE_MESSAGE_FLAG && (
                        <Button
                            ...
                            className='clarify-canvas-hints-hide-button'
                            type='link'
                        >
                            Hide
                        </Button>
        ...
        return (
            <div className='clarify-canvas-hints-container clarify-canvas-hints-container-disabled' />
        );
    }
}