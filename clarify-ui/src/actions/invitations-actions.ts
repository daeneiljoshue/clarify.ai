...
import { Invitation, Organization, getCore } from 'clarify-core-wrapper';
...
export function getInvitationsAsync(query: InvitationsQuery, showNotification = false): ThunkAction {
    return async function (dispatch, getState) {
        ...
        try {
            const invitations = await clarify.organizations.invitations({
                ...query,
...
export const acceptInvitationAsync = (
    ...
    try {
        const orgSlug = await clarify.organizations.acceptInvitation(key);
...
export const declineInvitationAsync = (
    ...
    try {
        await clarify.organizations.declineInvitation(key);
