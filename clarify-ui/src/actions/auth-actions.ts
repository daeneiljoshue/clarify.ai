...
import { getCore, User } from 'clarify-core-wrapper';
...
export const registerAsync = (
    ...
    try {
        const user = await clarify.server.register(
            username,
            firstName,
            lastName,
            email,
...
export const loginAsync = (credential: string, password: string): ThunkAction => async (dispatch) => {
    ...
    try {
        await clarify.server.login(credential, password);
        const users = await clarify.users.get({ self: true });
...
export const logoutAsync = (): ThunkAction => async (dispatch) => {
    ...
    try {
        await clarify.organizations.deactivate();
        await clarify.server.logout();
...
export const authorizedAsync = (): ThunkAction => async (dispatch) => {
    try {
        ...
        const result = await clarify.server.authorized();
        if (result) {
            const userInstance = (await clarify.users.get({ self: true }))[0];
...
export const changePasswordAsync = (
    ...
    try {
        await clarify.server.changePassword(oldPassword, newPassword1, newPassword2);
...
export const requestPasswordResetAsync = (email: string): ThunkAction => async (dispatch) => {
    ...
    try {
        await clarify.server.requestPasswordReset(email);
...
export const resetPasswordAsync = (
    ...
    try {
        await clarify.server.resetPassword(newPassword1, newPassword2, uid, token);
