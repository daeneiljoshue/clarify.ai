...
Cypress.Commands.add('createEllipse', (createEllipseParams) => {
    ...
    cy.get('.clarify-draw-ellipse-popover').within(() => {
        cy.get('.ant-select-selection-item').then(($labelValue) => {
            selectedValueGlobal = $labelValue.text();
        });
        cy.contains('button', createEllipseParams.type).click();
    });
    cy.get('.clarify-canvas-container')
...
Cypress.Commands.add('createSkeleton', (skeletonParameters) => {
    ...
    cy.get('.clarify-draw-skeleton-popover').within(() => {
        cy.get('.ant-select-selection-item').then(($labelValue) => {
            selectedValueGlobal = $labelValue.text();
        });
...
Cypress.Commands.add('changeAppearance', (colorBy) => {
    cy.get('.clarify-appearance-color-by-radio-group').within(() => {
...
Cypress.Commands.add('createPolygon', (createPolygonParams) => {
    if (!createPolygonParams.reDraw) {
        ...
        cy.get('.clarify-draw-polygon-popover').within(() => {
            cy.get('.ant-select-selection-item').then(($labelValue) => {
                selectedValueGlobal = $labelValue.text();
            });
    ...
    createPolygonParams.pointsMap.forEach((element) => {
        cy.get('.clarify-canvas-container').click(element.x, element.y);
    });
...
Cypress.Commands.add('openSettings', () => {
    cy.get('.clarify-header-menu-user-dropdown').click();
    cy.get('.clarify-header-menu')
        .should('exist')
        .and('be.visible')
        .find('[role="menuitem"]')
        .filter(':contains("Settings")')
        .click();
    cy.get('.clarify-settings-modal').should('be.visible');
});
...
Cypress.Commands.add('closeSettings', () => {
    ...
    cy.get('.clarify-settings-modal').should('not.be.visible');
});
...
Cypress.Commands.add('changeWorkspace', (mode) => {
    cy.get('.clarify-workspace-selector').click();
    cy.get('.clarify-workspace-selector-dropdown').within(() => {
        cy.get(`.ant-select-item-option[title="${mode}"]`).click();
    });

    cy.get('.clarify-workspace-selector').should('contain.text', mode);
});

Cypress.Commands.add('changeLabelAAM', (labelName) => {
    cy.get('.clarify-workspace-selector').then((value) => {
        const cvatWorkspaceSelectorValue = value.text();
        if (cvatWorkspaceSelectorValue.includes('Attribute annotation')) {
            cy.get('.clarify-attribute-annotation-sidebar-basics-editor').within(() => {
...
Cypress.Commands.add('createCuboid', (createCuboidParams) => {
    ...
    cy.get('.clarify-draw-cuboid-popover').within(() => {
        cy.get('.ant-select-selection-item').then(($labelValue) => {
            selectedValueGlobal = $labelValue.text();
        });
    ...
    cy.get('.clarify-canvas-container').click(createCuboidParams.firstX, createCuboidParams.firstY);
    cy.get('.clarify-canvas-container').click(createCuboidParams.secondX, createCuboidParams.secondY);
    if (createCuboidParams.points === 'By 4 Points') {
        cy.get('.clarify-canvas-container').click(createCuboidParams.thirdX, createCuboidParams.thirdY);
        cy.get('.clarify-canvas-container').click(createCuboidParams.fourthX, createCuboidParams.fourthY);
    }
...
Cypress.Commands.add('updateAttributes', (attributes) => {
    ...
    cy.get('.clarify-new-attribute-button').click();
    cy.document().then((doc) => {
        const cvatAttributeInputsWrapperList = Array.from(doc.querySelectorAll('.clarify-attribute-inputs-wrapper'));
        for (let i = 0; i < cvatAttributeInputsWrapperList.length; i++) {
            cvatAttributeInputsWrapperId.push(cvatAttributeInputsWrapperList[i].getAttribute('clarify-attribute-id'));
        }

        const minId = Math.min(...cvatAttributeInputsWrapperId);

        cy.get(`[cvat-attribute-id="${minId}"]`).within(() => {
            cy.get('.clarify-attribute-name-input').type(attributes.name);
            cy.get('.clarify-attribute-type-input').click();
        });
        cy.get('.ant-select-dropdown:has(.clarify-attribute-type-input-select)')
            .not('.ant-select-dropdown-hidden')
            .should('exist').and('be.visible')
            .first()
            .within(() => {
                cy.get(`.clarify-attribute-type-input-${attributes.type.toLowerCase()}`).click();
            });

        if (['Number', 'Text'].includes(attributes.type)) {
            cy.get(`[clarify-attribute-id="${minId}"]`).within(() => {
                if (attributes.values !== '') {
                    cy.get('.clarify-attribute-values-input').type(attributes.values);
                } else {
                    cy.get('.clarify-attribute-values-input').clear();
                }
            });
        } else if (['Radio', 'Select'].includes(attributes.type)) {
            cy.get(`[clarify-attribute-id="${minId}"]`).within(() => {
                cy.get('.clarify-attribute-values-input').type(`${attributes.values}{Enter}`);
            ...
            cy.get(`[clarify-attribute-id="${minId}"]`).within(() => {
                cy.get('.clarify-attribute-values-input').click();
            });
            cy.get('.ant-select-dropdown')
                .not('.ant-select-dropdown-hidden')
                .first()
        ...
        if (attributes.mutable) {
            cy.get('.clarify-attribute-mutable-checkbox')
                .find('[type="checkbox"]')
...
Cypress.Commands.add('createPolyline', (createPolylineParams) => {
    ...
    cy.get('.clarify-draw-polyline-popover').within(() => {
        cy.get('.ant-select-selection-item').then(($labelValue) => {
            selectedValueGlobal = $labelValue.text();
        });
...
Cypress.Commands.add('openTaskMenu', (taskName, fromTaskPage) => {
    if (fromTaskPage) {
        cy.contains('.clarify-text-color', 'Actions').click();
    } else {
        cy.contains('strong', taskName).parents('.clarify-tasks-list-item').find('.clarify-menu-icon').click();
    }
});
...
Cypress.Commands.add('deleteTask', (taskName) => {
    let taskId = '';
    cy.contains('.clarify-item-task-name', new RegExp(`^${taskName}$`))
        .parents('.clarifyt-task-item-description')
        .find('.clarify-item-task-id')
        .then(($taskId) => {
            taskId = $taskId.text().replace(/[^\d]/g, '');
            cy.clickInTaskMenu('Delete', false, taskName);
            cy.get('.clarify-modal-confirm-delete-task')
    ...
    cy.contains('.clarify-item-task-name', new RegExp(`^${taskName}$`))
        .parents('.clarify-tasks-list-item')
        .should('have.attr', 'style')
...
Cypress.Commands.add('advancedConfiguration', (advancedConfigurationParams) => {
    ...
    if (advancedConfigurationParams.sourceStorage) {
        ...
        cy.get('.clarify-select-source-storage').within(() => {
            cy.get('.ant-select-selection-item').click();
        });
        cy.contains('.clarify-select-source-storage-location', sourceStorage.location).should('be.visible').click();

        if (sourceStorage.cloudStorageId) {
            cy.get('.clarify-search-source-storage-cloud-storage-field').click();
            cy.get('.clarify-cloud-storage-select-provider').click();
        }
    }
    ...
    if (advancedConfigurationParams.targetStorage) {
        ...
        cy.get('.clarify-select-target-storage').within(() => {
            cy.get('.ant-select-selection-item').click();
        });
        cy.contains('.clarify-select-target-storage-location', targetStorage.location).should('be.visible').click();

        if (targetStorage.cloudStorageId) {
            cy.get('.clarify-search-target-storage-cloud-storage-field').click();
            cy.get('.clarify-cloud-storage-select-provider').last().click();
        }
    }
});

Cypress.Commands.add('removeAnnotations', () => {
    cy.contains('.clarify-annotation-header-button', 'Menu').click();
    cy.get('.clarify-annotation-menu').within(() => {
        cy.contains('Remove annotations').click();
    });
    cy.get('.clarify-modal-confirm-remove-annotation').within(() => {
        cy.contains('button', 'Delete').click();
    });
});
...
Cypress.Commands.add(
    'uploadAnnotations', (
        ...
        cy.get('.clarify-modal-import-dataset').find('.cvat-modal-import-select').click();
        cy.contains('.clarify-modal-import-dataset-option-item', format).click();
        cy.get('.clarify-modal-import-select').should('contain.text', format);

        if (!useDefaultLocation) {
            cy.get('.clarify-modal-import-dataset')
                .find('.clarify-modal-import-switch-use-default-storage')
                .click();
            cy.get('.clarify-select-source-storage').within(() => {
                cy.get('.ant-select-selection-item').click();
            });
            cy.contains('.clarify-select-source-storage-location', sourceStorage.location)
                .should('be.visible')
                .click();
            if (sourceStorage.cloudStorageId) {
                cy.get('.clarify-search-source-storage-cloud-storage-field').click();
        ...
        if (sourceStorage && sourceStorage.cloudStorageId) {
            cy.get('.cvat-modal-import-dataset')
                .find('.clarify-modal-import-filename-input')
                .type(filePath);
        } else {
        ...
        cy.closeNotification('.clarify-notification-notice-import-annotation-start');
...
Cypress.Commands.add('changeColorViaBadge', (labelColor) => {
    cy.get('.clarify-label-color-picker')
...
Cypress.Commands.add('collectLabelsName', () => {
    ...
    cy.get('.clarify-constructor-viewer').should('exist');
    cy.document().then((doc) => {
        const labels = Array.from(doc.querySelectorAll('.clarify-constructor-viewer-item'));
...
Cypress.Commands.add('deleteLabel', (labelName) => {
    cy.contains('.clarify-constructor-viewer-item', new RegExp(`^${labelName}$`))
        .should('exist')
        .and('be.visible')
        .find('[aria-label="delete"]')
        .click();
    cy.intercept('DELETE', '/api/labels/*').as('deleteLabel');
    cy.get('.clarify-modal-delete-label')
        .should('be.visible')
        .first()
        .within(() => {
    ...
    cy.contains('.clarify-constructor-viewer-item', new RegExp(`^${labelName}$`)).should('not.exist');
});
...
Cypress.Commands.add('addNewLabel', ({ name, color }, additionalAttrs) => {
    ...
    if (color) {
        cy.get('.clarify-change-task-label-color-badge').click();
        cy.changeColorViaBadge(color);
        cy.get('.clarify-label-color-picker').should('be.hidden');
    }
    if (additionalAttrs) {
    ...
    cy.get('.clarify-constructor-viewer').should('be.visible');
    cy.contains('.clarify-constructor-viewer-item', new RegExp(`^${name}$`)).should('exist');
});

Cypress.Commands.add('addNewSkeletonLabel', ({ name, points }) => {
    cy.get('.clarify-constructor-viewer-new-skeleton-item').click();
    cy.get('.clarify-skeleton-configurator').should('exist').and('be.visible');

    cy.get('.clarify-label-constructor-creator').within(() => {
    ...
    cy.get('.clarify-skeleton-configurator-svg').then(($canvas) => {
        ...
        cy.get('.clarify-skeleton-configurator-svg').within(() => {
...
Cypress.Commands.add('checkCanvasSidebarColorEqualness', (id) => {
    cy.get(`#clarify-objects-sidebar-state-item-${id}`).then(($el) => {
        ...
        cy.get(`#clarify_canvas_shape_${id}`).should('have.attr', 'fill', hexColor);
    });
});
...
Cypress.Commands.add('addNewLabelViaContinueButton', (additionalLabels) => {
    cy.collectLabelsName().then((labelsNames) => {
        if (additionalLabels.some((el) => labelsNames.indexOf(el) === -1)) {
            cy.get('.clarify-constructor-viewer-new-item').click();
...
Cypress.Commands.add('createTag', (labelName) => {
    ...
    cy.get('.clarify-setup-tag-popover').within(() => {
        cy.get('button').click();
    });
});

Cypress.Commands.add('sidebarItemSortBy', (sortBy) => {
    cy.get('.clarify-objects-sidebar-ordering-selector').click();
    cy.get('.clarify-objects-sidebar-ordering-dropdown').within(() => {
...
Cypress.Commands.add('goToNextFrame', (expectedFrameNum) => {
    cy.get('.clarify-player-next-button').click();
    cy.get('.clarify-player-next-button').trigger('mouseout');
    cy.checkFrameNum(expectedFrameNum);
});

Cypress.Commands.add('goToPreviousFrame', (expectedFrameNum) => {
    cy.get('.clarify-player-previous-button').click();
    cy.get('.clarify-player-previous-button').trigger('mouseout');
    cy.checkFrameNum(expectedFrameNum);
});
...
Cypress.Commands.add('setJobState', (choice) => {
    ...
    cy.get('.clarify-annotation-menu-job-state-submenu')
        .should('not.have.class', 'ant-zoom-big').within(() => {
            cy.contains(choice).click();
        });
    cy.get('.clarify-modal-content-change-job-state')
        .should('be.visible')
        .within(() => {
            cy.contains('[type="button"]', 'Continue').click();
        });
    cy.get('.clarify-modal-content-change-job-state').should('not.exist');
    cy.get('.clarify-spinner').should('not.exist');
});

Cypress.Commands.add('setJobStage', (jobID, stage) => {
    cy.get('.clarify-task-job-list')
        .contains('a', `Job #${jobID}`)
        .parents('.cvat-job-item')
        .find('.cvlarify-job-item-stage').click();
    cy.get('.ant-select-dropdown')
        .should('be.visible')
        .not('.ant-select-dropdown-hidden')
        .within(() => {
...
Cypress.Commands.add('getObjectIdNumberByLabelName', (labelName) => {
    cy.document().then((doc) => {
        const stateItemLabelSelectorList = Array.from(
            doc.querySelectorAll('.clarify-objects-sidebar-state-item-label-selector'),
        );
        for (let i = 0; i < stateItemLabelSelectorList.length; i++) {
            if (stateItemLabelSelectorList[i].textContent === labelName) {
                cy.get(stateItemLabelSelectorList[i])
                    .parents('.clarify-objects-sidebar-state-item')
                    .should('have.attr', 'id')
...
Cypress.Commands.add('closeModalUnsupportedPlatform', () => {
    if (Cypress.browser.family !== 'chromium' && !window.localStorage.getItem('platformNotiticationShown')) {
        cy.get('.clarify-modal-unsupported-platform-warning').within(() => {
            cy.contains('button', 'OK').click();
        });
    }
});
...
Cypress.Commands.add('exportTask', ({
    ...
    cy.get('.clarify-modal-export-task').should('be.visible').find('.clarify-modal-export-select').click();
    cy.contains('.clarify-modal-export-option-item', format).should('be.visible').click();
    cy.get('.clarify-modal-export-task').find('.clarify-modal-export-select').should('contain.text', format);
    if (type === 'dataset') {
        cy.get('.clarify-modal-export-task').find('.clarify-modal-export-save-images').should('not.be.checked').click();
    }
    if (archiveCustomName) {
        cy.get('.clarify-modal-export-task').find('.clarify-modal-export-filename-input').type(archiveCustomName);
    }
    cy.contains('button', 'OK').click();
    cy.get('.clarify-notification-notice-export-task-start').should('be.visible');
    cy.closeNotification('.clarify-notification-notice-export-task-start');
});
...
Cypress.Commands.add('exportJob', ({
    ...
    cy.get('.clarify-modal-export-job').should('be.visible').find('.clarify-modal-export-select').click();
    cy.contains('.clarify-modal-export-option-item', format).should('be.visible').click();
    cy.get('.clarify-modal-export-job').find('.clarify-modal-export-select').should('contain.text', format);
    if (type === 'dataset') {
        cy.get('.clarify-modal-export-job').find('.clarify-modal-export-save-images').should('not.be.checked').click();
    }
    if (archiveCustomName) {
        cy.get('.clarify-modal-export-job').find('.cvat-modal-export-filename-input').type(archiveCustomName);
    }
    if (!useDefaultLocation) {
        cy.get('.clarify-modal-export-job').find('.cvat-settings-switch').click();
        cy.get('.clarify-select-target-storage').within(() => {
            cy.get('.ant-select-selection-item').click();
        });
    ...
    cy.contains('button', 'OK').click();
    cy.get('.clarify-notification-notice-export-job-start').should('be.visible');
    cy.closeNotification('.clarify-notification-notice-export-job-start');
});

Cypress.Commands.add('renameTask', (oldName, newName) => {
    cy.get('.clarify-task-details-task-name').within(() => {
        cy.get('[aria-label="edit"]').click();
    });
    cy.contains('.clarify-text-color', oldName).type(`{selectall}{backspace}${newName}{Enter}`);
    cy.get('.clarify-spinner').should('not.exist');
    cy.contains('.clarify-task-details-task-name', newName).should('exist');
});
...
Cypress.Commands.add('shapeRotate', (shape, expectedRotateDeg, pressShift = false) => {
    ...
    cy.get(shape).should('have.class', 'clarify_canvas_shape_activated');
    cy.get('.svg_select_points_rot').then(($el) => {
        ...
        cy.get('.svg_select_points_rot').should('have.class', 'clarify_canvas_selected_point');
        ...
        cy.document().then((doc) => {
            ...
            cy.get('#clarify_canvas_text_content').should('contain.text', `${shapeTranformMatrix}°`);
...
Cypress.Commands.add('deleteFrame', (action = 'delete') => {
    ...
    if (action === 'restore') {
        cy.get('.clarify-player-restore-frame').click();
    } else if (action === 'delete') {
        cy.get('.clarify-player-delete-frame').click();
        cy.get('.clarify-modal-delete-frame').within(() => {
            cy.contains('button', 'Delete').click();
        });
    }
...
Cypress.Commands.add('deleteCloudStorage', (displayName) => {
    cy.get('.clarify-cloud-storage-item-menu-button').click();
    cy.get('.ant-dropdown')
        .not('.ant-dropdown-hidden')
        .within(() => {
            cy.contains('[role="menuitem"]', 'Delete').click();
        });
    cy.get('.clarify-delete-cloud-storage-modal')
...
Cypress.Commands.add('createJob', (options = {
    ...
    if (fromTaskPage) {
        cy.get('.clarify-create-job').click({ force: true });
    }
    cy.url().should('include', '/jobs/create');

    cy.get('.clarify-select-job-type').click();
    cy.get('.ant-select-dropdown')
        .not('.ant-select-dropdown-hidden')
        .first()
        .within(() => {
    ...
    cy.get('.clarify-select-frame-selection-method').click();
    cy.get('.ant-select-dropdown')
        .not('.ant-select-dropdown-hidden')
        .first()
        .within(() => {
    ...
    if (quantity) {
        cy.get('.clarify-input-frame-quantity').clear();
        cy.get('.clarify-input-frame-quantity').type(quantity);
    } else if (frameCount) {
        cy.get('.clarify-input-frame-count').clear();
        cy.get('.clarify-input-frame-count').type(frameCount);
    }

    if (seed) {
        cy.get('.clarify-input-seed').clear();
        cy.get('.clarify-input-seed').type(seed);
    }
...
Cypress.Commands.add('deleteJob', (jobID) => {
    cy.get('.clarify-job-item').contains('a', `Job #${jobID}`)
        .parents('.clarify-job-item')
        .find('.clarify-job-item-more-button')
        .click();
    cy.get('.ant-dropdown')
        .not('.ant-dropdown-hidden')
        .within(() => {
    ...
    cy.get('.clarify-modal-confirm-delete-job')
...
Cypress.Commands.add('drawMask', (instructions) => {
    for (const instruction of instructions) {
        ...
        if (method === 'brush-size') {
            ...
            cy.get('.clarify-brush-tools-brush').click();
            cy.get('.clarify-brush-tools-brush-size').within(() => {
                cy.get('input').clear();
            ...
            if (['brush', 'eraser'].includes(method)) {
                if (method === 'eraser') {
                    cy.get('.clarify-brush-tools-eraser').click();
                } else {
                    cy.get('.clarify-brush-tools-brush').click();
                }

                cy.get('.clarify-canvas-container').then(([$canvas]) => {
                ...
                if (method === 'polygon-plus') {
                    cy.get('.clarify-brush-tools-polygon-plus').click();
                } else {
                    cy.get('.clarify-brush-tools-polygon-minus').click();
                }
...
Cypress.Commands.add('startMaskDrawing', () => {
    cy.get('.clarify-draw-mask-control ').click();
    cy.get('.clarify-draw-mask-popover').should('exist').and('be.visible').within(() => {
        cy.get('button').click();
    });
    cy.get('.clarify-brush-tools-toolbox').should('exist').and('be.visible');
});

Cypress.Commands.add('finishMaskDrawing', () => {
    cy.get('.clarify-brush-tools-brush').click();
    cy.get('.clarify-brush-tools-finish').click();
});
...
Cypress.Commands.add('sliceShape', (
    ...
    if (shortcut) {
        ...
        cy.get('.clarify-slice-control').click();
        cy.get('.clarify-slice-control').trigger('mouseleave');
    }

    cy.get('.clarify-canvas-hints-container').within(() => {
...
Cypress.Commands.add('joinShapes', (
    ...
    const interactWithTool = () => {
        if (shortcut) {
            ...
            cy.get('.clarify-join-control').click();
            cy.get('.clarify-join-control').trigger('mouseleave');
        }
    };
    interactWithTool();
...
Cypress.Commands.add('interactAnnotationObjectMenu', (parentSelector, button) => {
    ...
    cy.document().find('.clarify-object-item-menu').within(() => {
