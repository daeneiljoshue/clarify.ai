...
context('Autoborder feature.', () => {
    ...
    before(() => {
        ...
        cy.get('#clarify-objects-sidebar-state-item-3').find('[data-icon="eye"]').click();
        cy.get('#clarify_canvas_shape_3').should('be.hidden');
    });

    describe(`Testing case "${caseId}"`, () => {
        it('Drawning a polygon with autoborder.', () => {
            // Collect the rectagle points coordinates
            testCollectCoord('rect', '#clarify_canvas_shape_1', rectanglePoints);

            cy.interactControlButton('draw-polygon');
            cy.get('.clarify-draw-polygon-popover').find('[type="button"]').contains('Shape').click();
            cy.get('body').type('{Ctrl}'); // Autoborder activation
            testAutoborderPointsCount(8); // 8 points at the rectangles
            cy.get('.clarify-canvas-container').click(400, 350);
            cy.get('.clarify-canvas-container').click(450, 250);
            cy.get('.clarify-canvas-container').click(500, 350);
            cy.get('.clarify-canvas-container').click(500, 450);
            ...
            cy.get('.clarify_canvas_autoborder_point').should('not.exist');

            // Collect the polygon points coordinates
            testCollectCoord('polygon', '#clarify_canvas_shape_4', polygonPoints);
        });

        it('Start drawing a polyline with autobordering between the two shapes.', () => {
            cy.interactControlButton('draw-polyline');
            cy.get('.clarify-draw-polyline-popover').find('[type="button"]').contains('Shape').click();
            testAutoborderPointsCount(12); // 8 points at the rectangles + 4 at the polygon
            cy.get('.clarify-canvas-container').click(600, 350);
            cy.get('.clarify-canvas-container').click(400, 450);
            cy.get('.clarify-canvas-container').click(550, 500);
            cy.get('.clarify-canvas-container').click(600, 450);
            cy.get('.clarify-canvas-container').click(600, 350);
            cy.get('.clarify-canvas-container').trigger('keydown', { keyCode: keyCodeN, code: 'KeyN' });
            cy.get('.clarify-canvas-container').trigger('keyup', { keyCode: keyCodeN, code: 'KeyN' });
            cy.get('.clarify_canvas_autoborder_point').should('not.exist');
            ...
            // Collect the polygon points coordinates
            testCollectCoord('polyline', '#clarify_canvas_shape_5', polylinePoints);
        });
