...
from clarify.apps.dataset_manager.task import _merge_table_rows
...
def process_shapes(db_job, apps, db_labels, db_attributes, db_alias):
    ...
    for shape_type in ['boxes', 'points', 'polygons', 'polylines']:
        for shape in get_old_db_shapes(shape_type, db_job):
            ...
            if shape_type == 'boxes':
                new_db_shape.type = clarify.apps.engine.models.ShapeType.RECTANGLE
                new_db_shape.points = [shape.xtl, shape.ytl, shape.xbr, shape.ybr]
            else:
                new_db_shape.points = shape.points.replace(',', ' ').split()
                if shape_type == 'points':
                    new_db_shape.type = clarify.apps.engine.models.ShapeType.POINTS
                elif shape_type == 'polygons':
                    new_db_shape.type = clarify.apps.engine.models.ShapeType.POLYGON
                elif shape_type == 'polylines':
                    new_db_shape.type = clarify.apps.engine.models.ShapeType.POLYLINE
    ...
    for track in tracks:
        ...
        for shape in track.shapes:
            ...
            if track.type == 'box_path':
                db_shape.type = clarify.apps.engine.models.ShapeType.RECTANGLE
                ...
                if track.type == 'points_path':
                    db_shape.type = clarify.apps.engine.models.ShapeType.POINTS
                elif track.type == 'polygon_path':
                    db_shape.type = clarify.apps.engine.models.ShapeType.POLYGON
                elif track.type == 'polyline_path':
                    db_shape.type = clarify.apps.engine.models.ShapeType.POLYLINE
...
def _save_old_shapes_to_db(apps, db_shapes, db_attributes, db_alias, db_job):
    ...
    shapes = [
        list(filter(lambda s: s.type == clarify.apps.engine.models.ShapeType.RECTANGLE, db_shapes)),
        list(filter(lambda s: s.type == clarify.apps.engine.models.ShapeType.POLYLINE, db_shapes)),
        list(filter(lambda s: s.type == clarify.apps.engine.models.ShapeType.POLYGON, db_shapes)),
        list(filter(lambda s: s.type == clarify.apps.engine.models.ShapeType.POINTS, db_shapes)),
    ]
    for i, shape_type in enumerate(['boxes', 'polylines', 'polygons', 'points']):
        ...
        for shape in shapes[i]:
            ...
            if shape.type == clarify.apps.engine.models.ShapeType.RECTANGLE:
            ...
            for attr in list(shape.labeledshapeattributeval_set.all()):
                ...
                if shape.type == clarify.apps.engine.models.ShapeType.POLYGON:
                    db_attrval.polygon_id = len(new_db_shapes)
                elif shape.type == clarify.apps.engine.models.ShapeType.POLYLINE:
                    db_attrval.polyline_id = len(new_db_shapes)
                elif shape.type == clarify.apps.engine.models.ShapeType.RECTANGLE:
...
def _save_old_tracks_to_db(apps, db_shapes, db_attributes, db_alias, db_job):
    ...
    tracks = [
        list(filter(lambda t: t.trackedshape_set.first().type == clarify.apps.engine.models.ShapeType.RECTANGLE, db_shapes)),
        list(filter(lambda t: t.trackedshape_set.first().type == clarify.apps.engine.models.ShapeType.POLYLINE, db_shapes)),
        list(filter(lambda t: t.trackedshape_set.first().type == clarify.apps.engine.models.ShapeType.POLYGON, db_shapes)),
        list(filter(lambda t: t.trackedshape_set.first().type == clarify.apps.engine.models.ShapeType.POINTS, db_shapes)),
    ]
    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('value', clarify.apps.engine.models.SafeCharField(max_length=64)),
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('points', clarify.apps.engine.models.FloatArrayField()),
                ('id', models.BigAutoField(primary_key=True, serialize=False)),
