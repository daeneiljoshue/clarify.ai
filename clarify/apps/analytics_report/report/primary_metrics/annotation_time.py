...
from clarify.apps.analytics_report.models import ViewChoice
from clarify.apps.analytics_report.report.primary_metrics.base import PrimaryMetricBase
...
class JobAnnotationTime(PrimaryMetricBase):
    ...
    _query = "SELECT timestamp, obj_val  FROM clarify.events WHERE scope='update:job' AND job_id={job_id:UInt64} AND obj_name='state' ORDER BY timestamp ASC"
