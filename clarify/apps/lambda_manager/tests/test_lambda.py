...
from clarify.apps.engine.tests.utils import filter_dict, get_paginated_collection
...
class _LambdaTestCaseBase(APITestCase):
    def setUp(self):
        ...
        http_patcher = mock.patch('clarify.apps.lambda_manager.views.LambdaGateway._http', side_effect = self._get_data_from_lambda_manager_http)
        self.addCleanup(http_patcher.stop)
        http_patcher.start()

        invoke_patcher = mock.patch('clarify.apps.lambda_manager.views.LambdaGateway.invoke', side_effect = self._invoke_function)
