...
CLARIFY_VERSION_PATTERN = re.compile(r'VERSION\s*=\s*\((\d+),\s*(\d*),\s*(\d+),\s*[\',\"](\w+)[\',\"],\s*(\d+)\)')

REPO_ROOT_DIR = Path(__file__).resolve().parents[1]

CLARIFY_INIT_PY_REL_PATH = 'cvat/__init__.py'
CLARIFY_INIT_PY_PATH = REPO_ROOT_DIR / CLARIFY_INIT_PY_REL_PATH
...
REPLACEMENT_RULES = [
    ReplacementRule(CLARIFY_INIT_PY_REL_PATH, CLARIFY_VERSION_PATTERN,
        lambda v, m: f'VERSION = {v.cvat_repr()}'),

    ReplacementRule('docker-compose.yml',
        re.compile(r'(\$\{CLARIFY_VERSION:-)([\w.]+)(\})'),
        lambda v, m: m[1] + v.compose_repr() + m[3]),

    ReplacementRule('helm-chart/values.yaml',
        re.compile(r'(^    image: clarify/(?:ui|server)\n    tag: )([\w.]+)', re.M),
        lambda v, m: m[1] + v.compose_repr()),

    ReplacementRule('clarify-sdk/gen/generate.sh',
        re.compile(r'^VERSION="[\d.]+"$', re.M),
        lambda v, m: f'VERSION="{v.major}.{v.minor}.{v.patch}"'),

    ReplacementRule('clarify/schema.yml',
        re.compile(r"^  version: [\d.]+$", re.M),
        lambda v, m:  f'  version: {v.major}.{v.minor}.{v.patch}'),

    ReplacementRule('clarify-cli/src/cvat_cli/version.py',
        re.compile(r'^VERSION = "[\d.]+"$', re.M),
        lambda v, m: f'VERSION = "{v.major}.{v.minor}.{v.patch}"'),

    ReplacementRule('clarify-cli/requirements/base.txt',
        re.compile(r'^clarify-sdk~=[\d.]+$', re.M),
        lambda v, m: f'clarify-sdk~={v.major}.{v.minor}.{v.patch}'),
]

def get_current_version() -> Version:
    version_text = CLARIFY_INIT_PY_PATH.read_text()

    match = re.search(CLARIFY_VERSION_PATTERN, version_text)
    if not match:
        raise RuntimeError(f'Failed to find version in {CLARIFY_INIT_PY_PATH}')

    return Version(int(match[1]), int(match[2]), int(match[3]), match[4], int(match[5]))

def main() -> None:
    parser = argparse.ArgumentParser(description='Bump CLARIFY version')
