...
describe('Feature: get a list of tasks', () => {
    test('get all tasks', async () => {
        const result = await clarify.tasks.get();
    ...
    test('get a task with skeletons by an id', async () => {
        ...
        for (const label of result[0].labels) {
            expect(label).toBeInstanceOf(clarify.classes.Label);
            if (label.type === 'skeleton') {
                ...
                for (const sublabel of label.structure.sublabels) {
                    expect(sublabel).toBeInstanceOf(clarify.classes.Label);
...
describe('Feature: save a task', () => {
    test('save some changed fields in a task', async () => {
        let result = await clarify.tasks.get({
            id: 2,
        });
        ...
        result = await clarify.tasks.get({
            id: 2,
        });
    ...
    test('save some new labels in a task', async () => {
        ...
        const newLabel = new clarify.classes.Label({
            name: "Another label",
            attributes: [
                {
                    default_value: 'false',
    ...
    test('save new task without an id', async () => {
        const task = new clarify.classes.Task({
            name: 'New Task',
            labels: [
                {
                    name: "My boss's car",
...
describe('Feature: delete a label', () => {
    test('delete a label', async () => {
        const [task] = await clarify.tasks.get({
            id: 100,
        });
        ...
        const [newlyRequestTask] = await clarify.tasks.get({
            id: 100,
        });
