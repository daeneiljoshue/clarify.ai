...
describe('Feature: get cloud storages', () => {
    ...
    test('get cloud storages by filters', async () => {
        ...
        const result = await clarify.cloudStorages.get({ filter: JSON.stringify(filter) });
...
describe('Feature: update a cloud storage', () => {
    test('update cloud storage with some new field values', async () => {
        ...
        let result = await clarify.cloudStorages.get({
            id: 1,
        });
        ...
        result = await clarify.cloudStorages.get({
            id: 1,
        });
        [cloudStorage] = result;
