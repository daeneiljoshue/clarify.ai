...
describe('Feature: get annotations', () => {
    ...
    test('get annotations from a job', async () => {
        const job = (await clarify.jobs.get({ jobID: 101 }))[0];
    ...
    test('get annotations for frame out of task', async () => {
        const task = (await clarify.tasks.get({ id: 100 }))[0];

        // Out of task
        expect(task.annotations.get(500)).rejects.toThrow(clarify.exceptions.ArgumentError);

        // Out of task
        expect(task.annotations.get(-1)).rejects.toThrow(clarify.exceptions.ArgumentError);
    });
    ...
    test('get annotations for frame out of job', async () => {
        ...
        // Out of segment
        expect(job.annotations.get(500)).rejects.toThrow(clarify.exceptions.ArgumentError);

        // Out of segment
        expect(job.annotations.get(-1)).rejects.toThrow(clarify.exceptions.ArgumentError);
    });
    ...
    test('get skeletons with a filter', async () => {
        const job = (await clarify.jobs.get({ jobID: 40 }))[0];
        ...
        for (const object of annotations) {
            ...
            object.elements.forEach((element, idx) => {
                expect(element).toBeInstanceOf(clarify.classes.ObjectState);
