...
describe('Feature: get frame meta', () => {
    ...
    test('get meta for a job', async () => {
        const job = (await clarify.jobs.get({ jobID: 100 }))[0];
    ...
    test('pass frame number out of a task', async () => {
        const task = (await clarify.tasks.get({ id: 100 }))[0];
        expect(task.frames.get(100)).rejects.toThrow(clarify.exceptions.ArgumentError);
        expect(task.frames.get(-1)).rejects.toThrow(clarify.exceptions.ArgumentError);
    });

    test('pass bad frame number', async () => {
        const task = (await clarify.tasks.get({ id: 100 }))[0];
        expect(task.frames.get('5')).rejects.toThrow(clarify.exceptions.ArgumentError);
    });

    test('do not pass any frame number', async () => {
        const task = (await clarify.tasks.get({ id: 100 }))[0];
        expect(task.frames.get()).rejects.toThrow(clarify.exceptions.ArgumentError);
    });
});
